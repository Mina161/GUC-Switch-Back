...
app.post("/login",upload.none(), async function (req, res) {
  ...
  if (found === null) {
    res.writeHead(404, { "Content-Type": "text/plain", "Access-Control-Allow-Headers": "http://localhost:3000/login" });
    res.write("User not found");
    res.end();
  } else {
    res.writeHead(200, { "Content-Type": "json", "Access-Control-Allow-Headers": "http://localhost:3000/login" });
